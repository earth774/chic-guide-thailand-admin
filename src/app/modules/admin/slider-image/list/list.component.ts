...
export class SliderImageListComponent extends BaseComponent implements OnInit, OnDestroy {

  @ViewChild('matDrawer', { static: true }) matDrawer: MatDrawer;

  public drawerMode: 'side' | 'over';
  public selectedSliderImage: SliderImage;
  public sliderImages: SliderImage[] = [];
  public sliderImageCount: number;
  ...
  setupEvents(): void {
    ...
    this.sliderImageService.sliderImageCreated$
      .pipe(takeUntil(this.unsubscribeAll))
      .subscribe(sliderImage => {
        this.sliderImages = [sliderImage, ...this.sliderImages];
        this.sliderImageCount++;
        this.pagination.length = this.sliderImageCount;
    ...
    this.sliderImageService.sliderImageDestroyed$
      .pipe(takeUntil(this.unsubscribeAll))
      .subscribe(sliderImage => {
        this.sliderImages = this.sliderImages.filter(c => c.id !== sliderImage.id);
        this.sliderImageCount--;
        this.pagination.length = this.sliderImageCount;
  ...
  async loadSliderImages() {
    try {
      ...
      this.changeDetectorRef.markForCheck();
      const { results, count } = await this.sliderImageService.find(this.queryParams);
      this.sliderImages = results;
      this.sliderImageCount = count;
